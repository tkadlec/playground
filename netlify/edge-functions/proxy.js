...
function cookieVal(source, name) {
  ...
  async function handleRequest(request) {
    ...
    if (acceptHeader && acceptHeader.indexOf('text/html') >= 0) {
      ...
      for( var i = 0; i < recipeList.length; i++ ){
        ...
        if (recipeType === 'asyncjs') {
          response = new HTMLRewriter()
            .on('script[src]', new AsyncJS(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'imageaspectratio') {
          response = new HTMLRewriter()
            .on('img', new ImageAspectRatio(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'fontdisplayswap') {
          ...
          response = new HTMLRewriter()
            .on('link[rel="stylesheet"],style,script', new FontSwap())
            .transform(response)
        }
        ...
        if (recipeType === 'insertheadstart') {
          ...
          response = new HTMLRewriter()
            .on('head', new Insert("start", htmltext))
            .transform(response)
        }
        ...
        if (recipeType === 'insertheadend') {
          let htmltext = decodeURIComponent(recipeIngredients[0]);
          response = new HTMLRewriter()
            .on('head', new Insert("end", htmltext))
            .transform(response)
        }
        ...
        if (recipeType === 'insertbodyend') {
          let htmltext = decodeURIComponent(recipeIngredients[0]);
          response = new HTMLRewriter()
            .on('body', new Insert("end", htmltext))
            .transform(response)
        }
        ...
        if (recipeType === 'asynccss' ) {
          response = new HTMLRewriter()
            .on('link[rel="stylesheet"]', new AsyncCSS(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'addimportance') {
          response = new HTMLRewriter()
            .on('script,img,link', new Importance(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'removeloadinglazy') {
          response = new HTMLRewriter()
            .on('img[loading=lazy]', new RemoveLoadingLazy(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'addpreload') {
          response = new HTMLRewriter()
            .on('head', new AddPreload(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'removepreload') {
          response = new HTMLRewriter()
            .on('link[rel=preload]' , new RemovePreload(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'addpreconnect') {
          response = new HTMLRewriter()
            .on('head', new AddPreconnect(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'disablescripts') {
          response = new HTMLRewriter()
            .on('script', new Scrapped(recipeIngredients))
            .transform(response)
        }
        ...
        if (recipeType === 'inline' && recipeIngredients.length) {
          ...
          const responses = await Promise.all(recipeIngredients.map(u => {
    ...
    // }
    //images tbd
    // else if (acceptHeader && acceptHeader.match('image/(png|jpg|gif)') ) {
    //   return fetch(url.toString(), request)
    // }
  ...
  class Importance {
    ...
    element(element) {
      if (this.ingredients) {
        let matchingIngredient = this.ingredients.filter(m => { return m.includes(element.getAttribute('src')) || m.includes(element.getAttribute('href')); } );
          if ( matchingIngredient.length ) {
            matchingIngredient = matchingIngredient[0];
